          ...
          return (
            ...
            >
              ...
              >
                ...
                <div className="text-sm leading-relaxed line-clamp-2 mb-3" style={{ color: T.textMuted }}>{term.description}</div>
                {/* P0 · Formula strip — 카드 스캔만으로 개념·단위 즉시 인식 */}
                {term.formula && (
                  <div className="mono text-[11px] md:text-xs px-2.5 py-1.5 border-l-2 mb-3 truncate"
                    style={{ background: T.bgSurface, borderColor: color?.bg, color: T.textSecondary, letterSpacing: '0.02em' }}
                    title={term.formula}
                  >
                    <span className="opacity-50 mr-1.5">ƒ</span>{term.formula}
                  </div>
                )}
                {/* P0 · Related 인라인 칩 — 관련 용어 최대 3개 */}
                {term.related && term.related.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-1">
                    {term.related.slice(0, 3).map(rid => (
                      <span key={rid} className="mono text-[10px] uppercase tracking-wider px-1.5 py-0.5 border"
                        style={{ borderColor: T.border, color: T.textMuted }}>
                        {rid}
                      </span>
                    ))}
                    {term.related.length > 3 && (
                      <span className="mono text-[10px] tracking-wider px-1 py-0.5" style={{ color: T.textFaint }}>
                        +{term.related.length - 3}
                      </span>
                    )}
                  </div>
      ...
      >
        ...
        <div className="flex flex-1 min-h-0">
          ...
          >
            ...
            <div className="p-5 border-b" style={{ borderColor: T.border }}>
              <div className="flex items-center gap-2 mb-3 text-[11px] mono uppercase tracking-[0.25em]" style={{ color: T.textFaint }}>
                <span style={{ color: T.accent }}>§</span> 목차
              </div>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, borderLeft: `1px solid ${T.border}` }}>
                {[
                  { label: '개요', key: 'summary' },
                  ...(term.detailed ? [{ label: '심화', key: 'detailed' }] : []),
                  { label: '공식', key: 'formula' },
                  { label: '예시', key: 'example' },
                  ...(term.relations && Object.keys(term.relations).length > 0 ? [{ label: '연결 관계', key: 'relations' }] : []),
                  ...(term.marketImpact ? [{ label: '시장 영향', key: 'impact' }] : []),
                  ...(relatedTerms.length > 0 ? [{ label: '관련 용어', key: 'related' }] : []),
                ].map((item, i) => (
                  <li
                    key={item.key}
                    style={{
                      ...
                      color: T.textMuted,
                      cursor: 'pointer',
                      borderLeft: i === 0 ? `2px solid ${categoryColors[term.category]?.bg || T.accent}` : '2px solid transparent',
                      marginLeft: '-1px',
                    }}
                  >
                    <span className="mono" style={{ fontSize: '10px', color: T.textDimmer, letterSpacing: '0.15em' }}>
            ...
            {/* 관련 용어 */}
            {relatedTerms.length > 0 && (
              <div className="p-5 border-b flex-1" style={{ borderColor: T.border }}>
                <div className="text-[11px] mono uppercase tracking-[0.25em] mb-3" style={{ color: T.textFaint }}>
                  관련 용어
                </div>
                  ...
                  {relatedTerms.slice(0, 6).map(rt => {
                    ...
                    return (
                      <button
                        key={rt.id}
                        onClick={() => onNavigate(rt.id)}
                        className="flex items-center gap-2 px-2.5 py-2 border text-left transition-all hover:bg-white/5"
                        style={{ borderColor: T.border, color: T.textSecondary, fontSize: '12.5px' }}
                      >
                        <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: rc?.bg }}></span>
                        <span className="flex-1 min-w-0 truncate">{rt.name}</span>
                        <ChevronRight size={10} style={{ color: T.textFaint, flexShrink: 0 }} />
                      </button>
                    );
                  })}
                </div>
            ...
            {term.id && (
              <div className="p-5 mt-auto">
                <button
                  className="w-full flex items-center gap-3 p-3 border text-left transition-all hover:bg-white/5"
                  style={{ borderColor: T.accent + '80', color: T.textSecondary }}
                  onClick={() => { onClose(); }}
                >
                  <span
                    className="mono text-[11px] font-semibold flex items-center justify-center shrink-0"
                    style={{ width: 28, height: 28, background: T.accent, color: '#0a0a0a', letterSpacing: '0.05em' }}
                  >
                    ƒ
                  </span>
                  <span>
                    <div style={{ fontSize: '12.5px', color: T.textPrimary }}>계산기 열기</div>
                    <div className="mono uppercase tracking-wider" style={{ fontSize: '10px', color: T.textFaint, marginTop: 2 }}>inline · shortcut</div>
                  </span>
                  <ChevronRight size={12} style={{ color: T.accent, marginLeft: 'auto' }} />
                </button>
              </div>
            )}
          </aside>
        ...
        >
          <button
            ...
            disabled={!hasPrev}
            className="flex items-center gap-2 text-[12px] mono uppercase tracking-[0.15em] transition-all hover:opacity-80"
            style={{ color: hasPrev ? T.textSecondary : T.textDimmer, cursor: hasPrev ? 'pointer' : 'not-allowed' }}
          >
            <ChevronLeft size={14} style={{ color: hasPrev ? T.accent : T.textDimmer }} />
            이전
          </button>
          {total > 0 && (
            <span className="mono text-[11px] tracking-[0.2em]" style={{ color: T.textFaint }}>
              {currentIdx + 1} / {total}
            </span>
          )}
          <button
            ...
            disabled={!hasNext}
            className="flex items-center gap-2 text-[12px] mono uppercase tracking-[0.15em] transition-all hover:opacity-80"
            style={{ color: hasNext ? T.textSecondary : T.textDimmer, cursor: hasNext ? 'pointer' : 'not-allowed' }}
          >
            다음
            <ChevronRight size={14} style={{ color: hasNext ? T.accent : T.textDimmer }} />
          </button>
        </div>
...
// ─────────────────────────────────────────────
// Market Pulse Rail (P3) — 상단 compact 지표 레일
// ─────────────────────────────────────────────
// 실시간 데이터 없이도 의미있는 인덱스 지표 4개를 compact하게 표시.
// 스파크라인은 SVG path로 시각적 리듬만 제공. (실제 API 연결 시 교체)
function MarketPulseRail({ T, totalTerms }: { T: any; totalTerms: number }) {
  ...
  // Index Rail 4칸 (정적 — API 연결 전 표시값)
  const indices = [
    { k: 'Terms', v: String(totalTerms).padStart(3, '0'), u: '개', bars: 4 },
    { k: 'Calcs',  v: '052',  u: '개', bars: 3 },
  ...
  return (
    ...
    >
      ...
      >
        <span className="mono text-[11px] tracking-[0.3em] uppercase" style={{ color: T.textDimmer }}>
          § Desk
        </span>
      </div>
            ...
            <div style={{ display: 'flex', gap: 2, marginTop: 2 }}>
              {Array.from({ length: 5 }).map((_, j) => (
                <span
                  key={j}
                  style={{
                    display: 'block', width: 4, height: 6,
                    background: j < idx.bars ? T.accent : T.border,
                    opacity: j < idx.bars ? 1 : 0.4,
                  }}
                />
              ))}
            </div>
...
// ─────────────────────────────────────────────
// A/B Diff 레이아웃 — DOM data-result-* 속성으로 두 시나리오 결과 비교
// ─────────────────────────────────────────────
function CalcABDiffLayout({ selectedCalc, renderCalcComponent, T }: any) {
  const refA = React.useRef<HTMLDivElement>(null);
  const refB = React.useRef<HTMLDivElement>(null);
  const [diffs, setDiffs] = React.useState<{ label: string; aVal: string; bVal: string; unit: string; diff: number | null; pct: number | null }[]>([]);

  // 300ms 폴링으로 두 패널의 결과를 읽어 diff 계산
  React.useEffect(() => {
    const readResults = (el: HTMLDivElement | null) => {
      if (!el) return [];
      return Array.from(el.querySelectorAll('[data-result-label]')).map(node => ({
        label: (node as HTMLElement).dataset.resultLabel || '',
        value: (node as HTMLElement).dataset.resultValue || '—',
        unit:  (node as HTMLElement).dataset.resultUnit  || '',
      }));
    };

    const tick = () => {
      const aRes = readResults(refA.current);
      const bRes = readResults(refB.current);
      if (!aRes.length) return;
      setDiffs(
        aRes.map((a, i) => {
          const b = bRes[i] || { value: '—', unit: a.unit };
          const aNum = parseFloat((a.value || '').replace(/,/g, ''));
          const bNum = parseFloat((b.value || '').replace(/,/g, ''));
          const diff    = (isFinite(aNum) && isFinite(bNum)) ? bNum - aNum : null;
          const pct     = (diff !== null && aNum !== 0) ? (diff / Math.abs(aNum)) * 100 : null;
          return { label: a.label, aVal: a.value, bVal: b.value, unit: a.unit, diff, pct };
        })
      );
    };
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 132px 1fr', overflow: 'hidden' }}>
      {/* A 시나리오 */}
      <div ref={refA} style={{ padding: '20px', borderRight: `1px solid ${T.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, letterSpacing: '0.22em', textTransform: 'uppercase', color: T.textFaint }}>
          <span style={{ padding: '2px 8px', background: T.accent, color: '#0a0a0a', fontSize: 11, letterSpacing: '0.15em' }}>A</span>
          시나리오 A
        </div>
      ...
      {/* Δ Diff 컬럼 */}
      <div style={{ background: T.bgCard, borderRight: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 8px', gap: 10 }}>
        <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, letterSpacing: '0.28em', textTransform: 'uppercase', color: T.textFaint, marginBottom: 4 }}>§ Δ</div>
        {diffs.map((d, i) => {
          const isUp = d.diff !== null && d.diff > 0;
          const isDn = d.diff !== null && d.diff < 0;
          const diffColor = isUp ? '#6f9c6a' : isDn ? '#b94040' : T.textDimmer;
          return (
            <div key={i} style={{ width: '100%', borderTop: i > 0 ? `1px dashed ${T.border}` : 'none', paddingTop: i > 0 ? 10 : 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
              <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: T.textDimmer, textAlign: 'center' }}>
                {d.label}
              </div>
              <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 15, fontWeight: 500, color: diffColor, textAlign: 'center' }}>
                {d.diff !== null ? (isUp ? '+' : '') + fmtDiff(d.diff) : '—'}
              </div>
              {d.pct !== null && (
                <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: diffColor, opacity: 0.8 }}>
        ...
        {diffs.length > 0 && (() => {
          // 핵심 결과(마지막 highlight 항목)의 판정
          const last = diffs[diffs.length - 1];
          if (last.diff === null) return null;
          const label = last.diff > 0 ? 'B가 높음' : last.diff < 0 ? 'B가 낮음' : '동일';
          const col = last.diff > 0 ? '#6f9c6a' : last.diff < 0 ? '#b94040' : T.textDimmer;
          return (
            <div style={{ marginTop: 'auto', width: '100%', borderTop: `1px solid ${T.border}`, paddingTop: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{ border: `1px solid ${col}`, color: col, fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '3px 6px', textAlign: 'center' }}>
                {label}
              </div>
              <div style={{ fontSize: 11, color: T.textMuted, textAlign: 'center', lineHeight: 1.5, fontStyle: 'italic' }}>
                {last.diff > 0
                  ? `B가 A보다 ${fmtDiff(Math.abs(last.diff))} ${last.unit} 높습니다`
                  : last.diff < 0
                    ? `B가 A보다 ${fmtDiff(Math.abs(last.diff))} ${last.unit} 낮습니다`
                    : '두 시나리오 결과가 동일합니다'}
              </div>
            </div>
          );
      ...
      {/* B 시나리오 */}
      <div ref={refB} style={{ padding: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, letterSpacing: '0.22em', textTransform: 'uppercase', color: T.textFaint }}>
          <span style={{ padding: '2px 8px', background: '#4A7045', color: '#fff', fontSize: 11, letterSpacing: '0.15em' }}>B</span>
          시나리오 B
        </div>
          ...
          {selectedCalc ? (
            ...
            >
              ...
              {compareCalcMode ? (
                <CalcABDiffLayout
                  selectedCalc={selectedCalc}
                  renderCalcComponent={renderCalcComponent}
                  T={T}
                />
              ) : (
...
function ResultBox({ label, value, unit, highlight, color = '#C89650', bands, interpret }: any) {
  ...
  return (
    ...
    >
      ...
      {/* P2 · 게이지 + 판정 + 해석 가이드 (highlight된 결과에 bands prop 있을 때만) */}
      {highlight && bands && (() => {
        const num = parseFloat((value || '').replace(/,/g, ''));
        if (!isFinite(num)) return null;
        const [low, high] = bands as [number, number];
        const range = high - low;
        // 0~100% 범위로 정규화 (양 끝 5% 여유)
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <div style={{ height: 4, background: 'rgba(0,0,0,0.25)', position: 'relative', marginBottom: 4 }}>
              <div style={{
                ...
                width: `${pct}%`,
                background: `linear-gradient(90deg, #6f9c6a 0%, #c89650 50%, #b94040 100%)`,
              }} />
              <div style={{
                position: 'absolute', top: -3, left: `${pct}%`,
                width: 2, height: 10, background: 'rgba(255,255,255,0.9)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{
                border: `1px solid ${judgeColor}`, color: judgeColor,
                fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, letterSpacing: '0.12em',
                textTransform: 'uppercase', padding: '2px 7px', background: 'rgba(0,0,0,0.2)',
              }}>◉ {judgeLabel}</span>
            </div>
            {/* 해석 가이드 */}
    ...
    <div>
      ...
      <div className="grid md:grid-cols-2 border" style={{ borderColor: _BORDER }}>
        ...
        <ResultBox label="PER · 주가수익비율" value={fmt(per)} unit="배" highlight
          bands={[8, 18]}
          interpret={(num: number, isLow: boolean, isHigh: boolean) =>
            isLow
              ? `PER ${num.toFixed(1)}배 — 한국 증시 평균(8~18배) 기준 저평가 구간. 업종 평균과 함께 비교하세요.`
              : isHigh
                ? `PER ${num.toFixed(1)}배 — 평균 이상 밸류에이션. 높은 성장 기대가 반영됐거나 고평가일 수 있습니다.`
                : `PER ${num.toFixed(1)}배 — 중립 범위. 업종·성장률 대비 적정 수준입니다.`
          }
        />
      </div>
      <CalcNote
...
function ROECalc() {
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 border" style={{ borderColor: _BORDER }}>
        <div className="md:border-r border-b md:border-b-0" style={{ borderColor: _BORDER }}><ResultBox label="ROE" value={fmt(roe)} unit="%" highlight color="#A63D33" bands={[10, 20]} interpret={(num: number, isLow: boolean, isHigh: boolean) => isLow ? `ROE ${num.toFixed(1)}% — 자본 효율성이 낮습니다. 업종 평균(10~20%) 하회.` : isHigh ? `ROE ${num.toFixed(1)}% — 우수한 자본 효율. 지속 가능성도 함께 점검하세요.` : `ROE ${num.toFixed(1)}% — 양호한 자본 효율 범위.`} /></div>
...
function SharpeCalc() {
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 border" style={{ borderColor: _BORDER }}>
        <div className="md:border-r border-b md:border-b-0" style={{ borderColor: _BORDER }}><ResultBox label="샤프지수" value={fmt(sharpe, 3)} unit="" highlight color="#4F7E7C" bands={[0.5, 2]} interpret={(num: number, isLow: boolean, isHigh: boolean) => isLow ? `샤프 ${num.toFixed(2)} — 위험 대비 수익이 낮습니다. 전략 개선을 고려하세요.` : isHigh ? `샤프 ${num.toFixed(2)} — 우수한 위험조정수익. 1 이상이면 시장 평균 상회.` : `샤프 ${num.toFixed(2)} — 양호한 위험조정수익 범위.`} /></div>
